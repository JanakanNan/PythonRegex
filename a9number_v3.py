...
def count_occurrences_in_text(word, text):
    ...
    """Set up a count for the occurence"""
    """pattern of regular expression in which given string supposed to be matched. """

    wordStart = "(?<![a-z])((?<!')|(?<=''))"
    wordEnd = "(?![a-z])((?!')|(?=''))"
    pattern = (re.match('[a-z]', word, re.I) is not None) * wordStart\
              + word \
              + (re.match('[a-z]', word[-1], re.I) is not None) * wordEnd
    return re.findall(pattern, text, re.IGNORECASE).__len__()
